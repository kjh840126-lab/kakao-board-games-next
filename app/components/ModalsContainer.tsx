  ...
  return (
    <>
      ...
      <div className={`fixed inset-0 z-50 transition-all duration-200 ${isAdminReportDrawerOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className={`absolute top-0 right-0 h-full w-4/5 max-w-sm flex flex-col shadow-2xl bg-white dark:bg-slate-900 text-slate-900 dark:text-white text-xs transition-transform duration-200 ease-out transform-gpu ${isAdminReportDrawerOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          <div 
          ...
          <div className="px-4 py-2.5 bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200/80 dark:border-slate-800 flex items-center text-xs">
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4">
            {selectedReport && (() => {
              ...
              return (
                <div className="p-3.5 rounded-2xl space-y-2.5 border bg-sky-50/70 dark:bg-sky-950/40 border-sky-300 dark:border-sky-800 shadow-xs">
                  <div className="flex justify-between items-center gap-2">
                    <span className="text-sky-800 dark:text-sky-300 font-extrabold bg-sky-200/80 dark:bg-sky-900/80 px-2 py-0.5 rounded-md text-[11px]">{sr.category || '기타'}</span>
                    <span className="text-slate-400 font-mono text-[11px]">{sr.userId || sr.user_id}</span>
                  </div>
                  <h3 className="font-extrabold text-slate-900 dark:text-white break-all leading-snug text-sm">{sr.title}</h3>
                  <p className="whitespace-pre-wrap break-all pt-2 border-t border-sky-200/80 dark:border-sky-900/80 text-slate-700 dark:text-slate-300 leading-relaxed text-xs">{sr.content || sr.description}</p>
                  <div className="pt-2 border-t border-sky-200/60 dark:border-sky-900/60 flex justify-end items-center">
                    {!isSrCompleted ? (
                      <button type="button" onClick={() => handleCompleteReport(selectedReport)} className="bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-white font-bold text-xs px-3 py-1.5 rounded-xl flex items-center gap-1.5 transition active:scale-95 cursor-pointer shadow-xs">
            ...
            <div className="space-y-2 pt-1">
              <h4 className="font-bold text-slate-400 dark:text-slate-500 text-xs">전체 목록 ({reports.length})</h4>
                  ...
                  <div key={currentReportId} onClick={() => handleMarkReportAsRead(report)} className={`p-3 rounded-xl border cursor-pointer transition flex items-center justify-between gap-2 ${isSelected ? 'border-sky-500 bg-sky-500 text-slate-900 font-bold shadow-xs' : isUnread ? 'border-amber-300 bg-amber-50/40 dark:bg-amber-950/20 hover:bg-amber-50 dark:hover:bg-amber-950/40' : 'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}>
                    <div className="flex items-center gap-1.5 min-w-0 flex-1">
                      {isUnread ? (
                        ...
                        <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-md flex-shrink-0 ${isSelected ? 'bg-sky-700 text-white' : 'bg-rose-100 text-rose-700 dark:bg-rose-900/80 dark:text-rose-300'}`}>미처리</span>
                      )}
                      <span className={`truncate text-xs ${isSelected ? 'text-slate-900 font-extrabold' : 'text-slate-800 dark:text-slate-200 font-medium'}`}>{r.title}</span>
                    </div>
                    <span className={`text-[10px] font-mono flex-shrink-0 ${isSelected ? 'text-slate-800' : 'text-slate-400 dark:text-slate-500'}`}>{r.userId || r.user_id}</span>
                  </div>
                );
              })}
            </div>
          ...
          <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
            <button onClick={() => setIsAdminReportDrawerOpen(false)} className="w-full bg-slate-900 dark:bg-slate-800 text-white py-2.5 rounded-xl font-bold cursor-pointer hover:bg-slate-800 dark:hover:bg-slate-700">닫기</button>
          </div>
        </div>
      </div>
      ...
      <div className={`fixed inset-0 z-50 transition-all duration-200 ${isSettingsOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className={`absolute top-0 right-0 h-full w-[40%] min-w-[260px] max-w-[320px] bg-white dark:bg-slate-900 flex flex-col justify-between shadow-2xl text-slate-900 dark:text-white text-xs transition-transform duration-200 ease-out transform-gpu ${isSettingsOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4">
            <div className="space-y-1">
              <h4 className="font-bold text-slate-400 dark:text-slate-500 text-[11px] px-1 mb-1">계정 설정</h4>
              <button 
                onClick={() => { 
                  if (currentUser) { 
                ...
                className="w-full py-2.5 px-2.5 rounded-xl text-left flex justify-between items-center cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-800 dark:text-slate-200 font-normal transition"
              >
            ...
            <div className="space-y-1">
              <h4 className="font-bold text-slate-400 dark:text-slate-500 text-[11px] px-1 mb-1">나의 활동</h4>
              <button 
              ...
              >
                ...
                <div className="flex items-center gap-1">
                  <span className="px-2 py-0.5 rounded-full bg-rose-50 dark:bg-rose-950/60 text-rose-600 dark:text-rose-300 font-extrabold text-[11px] border border-rose-100/80 dark:border-rose-800">
              ...
              >
                ...
                <div className="flex items-center gap-1">
                  <span className="px-2 py-0.5 rounded-full bg-amber-50 dark:bg-amber-950/60 text-amber-600 dark:text-amber-400 font-extrabold text-[11px] border border-amber-200/60 dark:border-amber-800">
            ...
            <div className="space-y-1">
              <h4 className="font-bold text-slate-400 dark:text-slate-500 text-[11px] px-1 mb-1">고객 지원</h4>
              <button 
            ...
            <div className="space-y-1">
              <h4 className="font-bold text-slate-400 dark:text-slate-500 text-[11px] px-1 mb-1">화면 설정</h4>
              
              <div className="flex justify-between items-center py-1 px-2.5">
                <div className="flex items-center gap-2.5 text-slate-800 dark:text-slate-200 font-normal">
                ...
                <div className="flex p-0.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-[11px]">
                  <button 
                    ...
                    className={`px-2.5 py-1 rounded-md font-normal transition cursor-pointer ${theme === 'light' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-2xs' : 'text-slate-400 dark:text-slate-500'}`}
                  >
                    라이트
                  </button>
                  <button 
                    onClick={() => setTheme && setTheme('dark')} 
                    className={`px-2.5 py-1 rounded-md font-normal transition cursor-pointer ${theme === 'dark' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-2xs' : 'text-slate-400 dark:text-slate-500'}`}
                  >
                    다크
                  </button>
                </div>
              ...
              <div className="flex justify-between items-center py-1 px-2.5">
                ...
                <div className="flex p-0.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-[11px]">
                  <button 
                    ...
                    className={`px-2.5 py-1 rounded-md font-normal transition cursor-pointer ${fontSize === 'normal' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-2xs' : 'text-slate-400 dark:text-slate-500'}`}
                  >
                    보통
                  </button>
                  <button 
                    onClick={() => setFontSize('large')} 
                    className={`px-2.5 py-1 rounded-md font-normal transition cursor-pointer ${fontSize === 'large' ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-2xs' : 'text-slate-400 dark:text-slate-500'}`}
                  >
                    크게
                  </button>
                </div>
            ...
            <div className="pt-1">
              <button 
                ...
                className="w-full py-2.5 px-2.5 rounded-xl text-left flex items-center gap-2.5 cursor-pointer text-rose-600 dark:text-rose-400 font-normal hover:bg-rose-50 dark:hover:bg-rose-950/40 transition"
              >
                <LogOut size={16} />
                <span>로그아웃</span>
              </button>
          ...
          <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
            <button onClick={() => setIsSettingsOpen(false)} className="w-full bg-slate-900 dark:bg-slate-800 text-white py-2.5 rounded-xl font-bold cursor-pointer hover:bg-slate-800 dark:hover:bg-slate-700">닫기</button>
          </div>
        </div>
      </div>
        ...
        <div className={`absolute top-0 right-0 h-full w-4/5 max-w-sm flex flex-col shadow-2xl bg-white dark:bg-slate-900 text-slate-900 dark:text-white text-xs transition-transform duration-200 ease-out transform-gpu ${isNoticeDrawerOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-3">
            ...
            {notices.map((notice: Notice) => {
              ...
              return (
                <div key={notice.noticeId} className={`rounded-2xl border overflow-hidden ${isExpanded ? 'border-amber-400 bg-amber-50/60 dark:bg-amber-950/30' : 'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900'}`}>
                  <div onClick={() => handleNoticeClick(notice)} className="p-3.5 cursor-pointer flex justify-between items-start gap-2">
                    <div className="flex-1 min-w-0"><h3 className="font-extrabold leading-snug break-all text-slate-900 dark:text-white text-xs">{notice.title}</h3><span className="text-slate-400 dark:text-slate-500 font-mono mt-1 block text-[10px]">{notice.createdAt}</span></div>
                    <ChevronDown size={18} className={`text-slate-400 dark:text-slate-500 transition-transform ${isExpanded ? 'rotate-180 text-amber-500' : ''}`} />
                  </div>
                  {isExpanded && (
                    <div className="px-3.5 pb-4 pt-2 border-t border-slate-200 dark:border-slate-800 font-medium leading-relaxed break-all text-slate-600 dark:text-slate-300 text-xs space-y-3">
                      {noticeImgUrl && (
                        <div className="w-full rounded-xl overflow-hidden border border-slate-200/80 dark:border-slate-800 bg-slate-100 dark:bg-slate-800 shadow-2xs">
                          <img 
                            src={noticeImgUrl} 
                            alt={notice.title} 
                      ...
                      <p className="whitespace-pre-wrap leading-normal text-slate-800 dark:text-slate-200 font-normal">{notice.content}</p>
                    </div>
                  )}
                </div>
              );
          ...
          <div className="p-4 border-t border-slate-200 dark:border-slate-800"><button onClick={() => setIsNoticeDrawerOpen(false)} className="w-full bg-slate-900 dark:bg-slate-800 text-white py-2.5 rounded-xl font-bold cursor-pointer text-xs hover:bg-slate-800 dark:hover:bg-slate-700">닫기</button></div>
        </div>
      </div>
      ...
      <div className={`fixed inset-0 z-50 transition-all duration-200 ${isCartOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className={`absolute top-0 right-0 h-full w-4/5 max-w-sm flex flex-col shadow-2xl bg-white dark:bg-slate-900 text-slate-900 dark:text-white text-xs transition-transform duration-200 ease-out transform-gpu ${isCartOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="p-4 border-b space-y-2 bg-slate-50 dark:bg-slate-800/60 border-slate-200/80 dark:border-slate-800">
            ...
            <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-none pt-1">
              {Array.from({ length: 14 }, (_, i) => i + 1).map((days: number) => (
                <button key={days} onClick={() => setRentalDays(days)} className={`px-3 py-1.5 rounded-xl font-bold whitespace-nowrap transition-all cursor-pointer ${rentalDays === days ? 'bg-slate-900 dark:bg-slate-700 text-white shadow-sm scale-105' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700'}`}>{days}일</button>
              ))}
            </div>
            <div className="text-slate-400 dark:text-slate-500 font-medium flex justify-between items-center pt-0.5">
              <span>반납 예정일:</span><strong className="text-slate-900 dark:text-white font-extrabold">{calculateEndDate()}</strong>
            </div>
          </div>
          <div className="flex-1 p-4 overflow-y-auto space-y-2.5">
            {cart.length === 0 ? <div className="text-center py-16 text-slate-400 dark:text-slate-500 font-medium">담긴 보드게임이 없습니다.</div> : cart.map((game: Game) => (
              <div key={game.gameId} className="flex justify-between items-center border p-3.5 rounded-xl shadow-sm bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700">
                <div><h4 className="font-bold text-slate-900 dark:text-white">{game.title}</h4><p className="text-slate-400 dark:text-slate-500 mt-0.5 text-[11px]">{game.minPlayers}~{game.maxPlayers}인 | {game.playTime}분</p></div>
                <button onClick={() => removeFromCart(game.gameId)} className="text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 p-1 cursor-pointer"><Trash2 size={16} /></button>
              </div>
            ))}
          </div>
      ...
      {isFavoritesModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative rounded-2xl w-full max-w-sm p-5 space-y-4 shadow-2xl border max-h-[85vh] flex flex-col bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 text-slate-900 dark:text-white text-xs">
            ...
            <div className="flex-1 overflow-y-auto space-y-2.5 pr-1">
              {favoriteGamesList.length === 0 ? <div className="text-center py-12 text-slate-400 dark:text-slate-500">찜한 보드게임이 없습니다.</div> : favoriteGamesList.map(game => (
                <div key={game.gameId} className="p-3 rounded-2xl border flex items-center justify-between gap-3 shadow-sm bg-slate-50 dark:bg-slate-800/60 border-slate-200 dark:border-slate-800">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <img src={game.imageUrl} alt={game.title} className="w-12 h-12 object-cover rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex-shrink-0" onError={(e) => { (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1610890716171-6b1bb98ffd09?w=300'; }} />
                    <div className="min-w-0 flex-1"><h4 className="font-bold truncate text-slate-900 dark:text-white">{game.title}</h4></div>
                  </div>
            ...
            <button onClick={() => setIsFavoritesModalOpen(false)} className="w-full bg-slate-900 dark:bg-slate-800 text-white py-2.5 rounded-xl font-bold cursor-pointer hover:bg-slate-800 dark:hover:bg-slate-700">닫기</button>
          </div>
        </div>
      )}
      ...
      {isMyRatingsModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative rounded-2xl w-full max-w-sm p-5 space-y-4 shadow-2xl border max-h-[85vh] flex flex-col bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 text-slate-900 dark:text-white text-xs">
            ...
            <div className="flex-1 overflow-y-auto space-y-2.5 pr-1">
              {myRatingGamesList.length === 0 ? <div className="text-center py-12 text-slate-400 dark:text-slate-500">평가를 남긴 보드게임이 없습니다.</div> : myRatingGamesList.map((game: any) => (
                <div key={game.gameId} className="p-3 rounded-2xl border flex items-center justify-between gap-3 shadow-sm bg-slate-50 dark:bg-slate-800/60 border-slate-200 dark:border-slate-800">
                  <div className="flex items-center gap-3 min-w-0">
                    <img src={game.imageUrl} alt={game.title} className="w-11 h-11 object-cover rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex-shrink-0" onError={(e) => { (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1610890716171-6b1bb98ffd09?w=300'; }} />
                    <div className="min-w-0"><h4 className="font-bold truncate text-slate-900 dark:text-white">{game.title}</h4><span className="text-rose-500 font-extrabold">{game.myScore?.toFixed(1)}점</span></div>
                  </div>
                  <button onClick={() => handleDeleteMyRating(game.gameId)} className="p-1 text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 cursor-pointer"><Trash2 size={14} /></button>
                </div>
              ))}
            </div>
            <button onClick={() => setIsMyRatingsModalOpen(false)} className="w-full bg-slate-900 dark:bg-slate-800 text-white py-2.5 rounded-xl font-bold cursor-pointer hover:bg-slate-800 dark:hover:bg-slate-700">닫기</button>
          </div>
        </div>
      )}
      ...
      {ratingModalGame && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative rounded-2xl w-full max-w-xs p-5 space-y-4 shadow-2xl border text-center bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 text-slate-900 dark:text-white text-xs">
            <div className="flex justify-between items-center pb-2 border-b border-slate-200/20 dark:border-slate-800">
              ...
              <button onClick={() => setRatingModalGame(null)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"><X size={16} /></button>
            </div>
            <div className="space-y-2">
              <h4 className="font-extrabold text-base text-slate-900 dark:text-white">{ratingModalGame.title}</h4>
              <p className="text-slate-400 dark:text-slate-500">별점을 선택해 주세요.</p>
              <div className="py-2"><span className="text-3xl font-black text-rose-500">{selectedScore.toFixed(1)}</span><span className="text-slate-400 dark:text-slate-500 font-bold text-sm"> / 5.0</span></div>
            ...
            <div className="flex gap-2 pt-2">
              <button type="button" onClick={() => setRatingModalGame(null)} className="flex-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 py-2.5 rounded-xl font-bold cursor-pointer">취소</button>
      ...
      {isReportModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative rounded-2xl w-full max-w-sm p-5 space-y-4 shadow-2xl border bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 text-slate-900 dark:text-white text-xs">
            <div className="flex justify-between items-center pb-2 border-b border-slate-200/20 dark:border-slate-800">
            ...
            <form onSubmit={handleSendReport} className="space-y-3.5">
              <div>
                ...
                <select value={reportForm.category} onChange={(e) => setReportForm({ ...reportForm, category: e.target.value })} className="w-full border border-slate-200 dark:border-slate-800 p-2.5 rounded-xl text-slate-900 dark:text-white bg-white dark:bg-slate-800/60 cursor-pointer">
              ...
              <div><label className="font-bold block mb-1.5">제목</label><input type="text" required placeholder="제목을 입력해 주세요" value={reportForm.title} onChange={(e) => setReportForm({ ...reportForm, title: e.target.value })} className="w-full border border-slate-200 dark:border-slate-800 p-2.5 rounded-xl text-slate-900 dark:text-white bg-white dark:bg-slate-800/60" /></div>
              <div><label className="font-bold block mb-1.5">상세 내용</label><textarea required rows={5} placeholder="내용을 작성해 주세요." value={reportForm.content} onChange={(e) => setReportForm({ ...reportForm, content: e.target.value })} className="w-full border border-slate-200 dark:border-slate-800 p-2.5 rounded-xl text-slate-900 dark:text-white bg-white dark:bg-slate-800/60 resize-none"></textarea></div>
              <div className="flex gap-2 pt-1">
                <button type="button" onClick={() => setIsReportModalOpen(false)} className="flex-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 py-2.5 rounded-xl font-bold cursor-pointer">취소</button>
            ...
            <form onSubmit={onSubmitProfile} className="space-y-3.5">
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full border border-slate-200 dark:border-slate-700 p-2.5 rounded-xl font-mono bg-slate-100 dark:!bg-slate-800/90 text-slate-500 dark:!text-slate-400 cursor-not-allowed select-none opacity-90" 
                />
              </div>

              {/* ⚡ 2. 이메일 항목 (다크모드 비활성화 명확화) */}
              <div>
                ...
                <input 
                  ...
                  className="w-full border border-slate-200 dark:border-slate-700 p-2.5 rounded-xl bg-slate-100 dark:!bg-slate-800/90 text-slate-500 dark:!text-slate-400 cursor-not-allowed select-none opacity-90" 
                />
              </div>
              ...
              <div>
                ...
                <input 
                  ...
                  } p-2.5 rounded-xl text-slate-900 dark:text-white bg-white dark:bg-slate-800/60 focus:outline-none focus:border-slate-400 dark:focus:border-slate-700`} 
                />
                {isNameHasNonKorean && (
              ...
              <div className="pt-2 border-t border-slate-200/20 dark:border-slate-800 space-y-2">
                ...
                <div>
                  <input 
                    ...
                    className={`w-full border ${
                      isProfilePasswordMismatch ? 'border-red-500' : 'border-slate-200 dark:border-slate-800'
                    } p-2.5 rounded-xl text-slate-900 dark:text-white bg-white dark:bg-slate-800/60 focus:outline-none focus:border-slate-400 dark:focus:border-slate-700`} 
                  />
              ...
              <div className="flex gap-2 pt-2">
                <button type="button" onClick={() => setIsEditProfileOpen(false)} className="flex-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 py-2.5 rounded-xl font-bold cursor-pointer">취소</button>
                <button type="submit" className="flex-1 bg-slate-900 dark:bg-slate-800 text-white py-2.5 rounded-xl font-bold cursor-pointer hover:bg-slate-800 dark:hover:bg-slate-700">저장하기</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {isGameModalOpen && editingGame && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative rounded-2xl w-full max-w-sm p-5 space-y-3.5 max-h-[90vh] overflow-y-auto shadow-2xl border bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 text-slate-900 dark:text-white text-xs">
            <div className="flex justify-between items-center pb-2 border-b border-slate-200/20 dark:border-slate-800">
            ...
            <form onSubmit={saveGame} className="space-y-3">
              ...
              <div className="flex gap-2">
                <div className="w-[35%]"><label className="font-bold block mb-1">보드게임 ID</label><input type="text" required disabled={isEditingMode} value={editingGame.gameId} onChange={(e) => setEditingGame({ ...editingGame, gameId: e.target.value })} className="w-full border border-slate-200 dark:border-slate-800 p-2.5 rounded-xl font-bold text-slate-900 dark:text-white bg-white dark:bg-slate-800/60 disabled:bg-slate-100 dark:disabled:bg-slate-800/80" /></div>
                <div className="w-[65%]"><label className="font-bold block mb-1">게임명</label><input type="text" required value={editingGame.title} onChange={(e) => setEditingGame({ ...editingGame, title: e.target.value })} className="w-full border border-slate-200 dark:border-slate-800 p-2.5 rounded-xl text-slate-900 dark:text-white bg-white dark:bg-slate-800/60" /></div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full border border-slate-200 dark:border-slate-800 p-2.5 rounded-xl text-slate-900 dark:text-white bg-white dark:bg-slate-800/60" 
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-4 gap-1.5">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full border border-slate-200 dark:border-slate-800 p-2 rounded-xl text-slate-900 dark:text-white bg-white dark:bg-slate-800/60" 
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <select value={editingGame.isVisible} onChange={(e) => setEditingGame({ ...editingGame, isVisible: e.target.value as 'Y' | 'N' })} className="w-full border border-slate-200 dark:border-slate-800 p-2 rounded-xl bg-slate-50 dark:bg-slate-800/60 text-slate-900 dark:text-white font-semibold cursor-pointer">
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full font-bold transition-all ${
                          isSelected
                            ...
                            : isDisabled
                            ? 'bg-slate-200/70 dark:bg-slate-800/40 text-slate-400 dark:text-slate-600 border border-slate-300 dark:border-slate-800 border-dashed cursor-not-allowed'
                            : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-200 dark:hover:bg-slate-700 cursor-pointer'
                        }`}
                      >
                        {genre}
                      </button>
              ...
              <div className="flex gap-2 pt-2"><button type="button" onClick={() => setIsGameModalOpen(false)} className="flex-1 bg-slate-100 dark:bg-slate-800 py-2.5 rounded-xl font-bold text-slate-700 dark:text-slate-200 cursor-pointer">취소</button><button type="submit" disabled={isUploadingImage} className="flex-1 bg-slate-900 dark:bg-slate-800 text-white py-2.5 rounded-xl font-bold cursor-pointer disabled:opacity-50 hover:bg-slate-800 dark:hover:bg-slate-700">저장</button></div>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative rounded-2xl w-full max-w-sm p-5 space-y-3.5 shadow-2xl border bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 text-slate-900 dark:text-white text-xs">
            ...
            <form onSubmit={saveNotice} className="space-y-3">
              <div><label className="font-bold block mb-1">공지 제목</label><input type="text" required value={editingNotice.title} onChange={(e) => setEditingNotice({ ...editingNotice, title: e.target.value })} className="w-full border border-slate-200 dark:border-slate-800 p-2.5 rounded-xl text-slate-900 dark:text-white bg-white dark:bg-slate-800/60" /></div>
              ...
              <div><label className="font-bold block mb-1">공지 내용</label><textarea required rows={4} value={editingNotice.content} onChange={(e) => setEditingNotice({ ...editingNotice, content: e.target.value })} className="w-full border border-slate-200 dark:border-slate-800 p-2.5 rounded-xl text-slate-900 dark:text-white bg-white dark:bg-slate-800/60 resize-none"></textarea></div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full border border-slate-200 dark:border-slate-800 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 text-slate-900 dark:text-white font-semibold cursor-pointer"
                >
              ...
              <div className="flex gap-2 pt-2"><button type="button" onClick={() => setIsNoticeModalOpen(false)} className="flex-1 bg-slate-100 dark:bg-slate-800 py-2.5 rounded-xl font-bold text-slate-700 dark:text-slate-200 cursor-pointer">취소</button><button type="submit" disabled={isUploadingNoticeImage} className="flex-1 bg-slate-900 dark:bg-slate-800 text-white py-2.5 rounded-xl font-bold cursor-pointer disabled:opacity-50 hover:bg-slate-800 dark:hover:bg-slate-700">저장</button></div>
            </form>
          </div>
        </div>
      )}

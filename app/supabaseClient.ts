...
/**
 * ⚡ 보드게임 이미지 파일 업로드 헬퍼 함수 (games/ 폴더 하위에 저장)
...
/**
 * ⚡ 공지사항 전용 이미지 파일 업로드 헬퍼 함수 (notices/ 폴더 하위에 저장)
 * @param file 업로드할 File 객체
 * @param bucketName Supabase Storage 버킷명 (기본값: 'game-images')
 * @returns 업로드 완료된 이미지의 Public URL (실패 시 null)
 */
export const uploadNoticeImage = async (file: File, bucketName: string = 'game-images'): Promise<string | null> => {
  try {
    // 1. 파일명 중복 방지용 고유 파일명 생성
    const fileExt = file.name.split('.').pop();
    const fileName = `${Date.now()}_${Math.random().toString(36).substring(2, 9)}.${fileExt}`;
    ...
    // 2. Supabase Storage의 notices/ 폴더에 파일 업로드
    const { data, error: uploadError } = await supabase.storage
      .from(bucketName)
      .upload(filePath, file, {
        cacheControl: '3600',
    ...
    if (uploadError) {
      console.error('⚠️ Supabase Storage 공지 이미지 업로드 오류:', uploadError.message);
      alert('공지 이미지 업로드에 실패했습니다: ' + uploadError.message);
      return null;
    }
    ...
    console.error('⚠️ 공지 이미지 업로드 중 예외 발생:', err);
